...
def vec_evaluate_episode_rtg(
    ...
    assert state.shape[0] == len(target_return)
    ...
    for t in range(max_ep_len):
        ...
        # if isinstance(vec_env.venv.env, LateralAndLongitduinalGTSWrapper):
        if 'phase_action' in infos[0]:
            taken_actions = np.array([info['phase_action'] for info in infos])
            action = torch.from_numpy(taken_actions.reshape(action.shape)).to(device=device)
...
def vec_evaluate_episode_rtg_OLD(
    vec_env,
    state_dim,
    act_dim,
    model,

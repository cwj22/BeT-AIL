...
from online_dt.decision_transformer.models.model import TrajectoryModel
from online_dt.decision_transformer.models.trajectory_gpt2 import GPT2Model
import math
import numpy as np
...
class DecisionTransformer(TrajectoryModel):
    ...
    def forward(
        ...
        n_stacked_inputs = 3 if self.return_embedding else 2
        ...
        if self.return_embedding:
            ...
            stacked_inputs = (
                ...
                .permute(0, 2, 1, 3)
                .reshape(batch_size, n_stacked_inputs * seq_length, self.hidden_size)
            )
        else:
        ...
        stacked_padding_mask = (
            torch.stack(tuple([padding_mask for _ in range(n_stacked_inputs)]), dim=1)
            .permute(0, 2, 1)
            .reshape(batch_size, n_stacked_inputs * seq_length)
        )
        ...
        x = x.reshape(batch_size, seq_length, n_stacked_inputs, self.hidden_size).permute(0, 2, 1, 3)
    ...
    def get_predictions(
        ...
        if self.stochastic_policy:
            if self.return_embedding:
                ...
                return state_preds[:, -1], action_preds, return_preds

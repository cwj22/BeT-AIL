...
class SequenceTrainer:
    ...
    def train_iteration(
        ...
        for i, trajs in enumerate(dataloader):
            if i%100 == 0:
                print(i, ' of ', len(dataloader), time.time()-t)
                t = time.time()
            loss, nll, entropy = self.train_step_stochastic(loss_fn, trajs, update_entropy)
            losses.append(loss)
            nlls.append(nll)
    ...
    def train_step_stochastic(self, loss_fn, trajs, update_entropy=True,):
        (
            states,
            actions,
            rewards,

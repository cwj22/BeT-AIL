...
def snwrap(use_sn=False, spectral_norm_kwargs=None):
    ...
    def _snwrap(m):
        ...
        if use_sn:
            return nn.utils.spectral_norm(m, **spectral_norm_kwargs)
        else:
            return m

    return _snwrap
#https://github.com/lionelblonde/liayn-pytorch/blob/39a10c88c81b240e3bcf16107a21b4bbd7827751/agents/nets.py
...
def build_mlp_spectral_norm(
    ...
    kw = spectral_norm_kwargs if spectral_norm_kwargs else {}
    apply_sn = snwrap(use_sn=True, spectral_norm_kwargs=kw)

    # Hidden layers
    prev_size = in_size
    for i, size in enumerate(hid_sizes):
        layers[f"{prefix}dense{i}"] = apply_sn(nn.Linear(prev_size, size))
        prev_size = size
        if activation:
            layers[f"{prefix}act{i}"] = activation(negative_slope=leak)
        if dropout_prob > 0.0:
            layers[f"{prefix}dropout{i}"] = nn.Dropout(dropout_prob)
    ...
    for k in layers:
        if 'dense' in k:
            if 'final' in k:
                layers[k].apply(init(weight_scale=0.01))
            else:
                layers[k].apply(init(weight_scale=math.sqrt(2) / math.sqrt(1 + leak ** 2)))

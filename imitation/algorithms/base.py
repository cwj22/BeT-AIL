...
class RandomSequentialSampler(th_data.Sampler):
    """
    Samples random batches of indices in a sequential order.
    """

    def __init__(self, data_source, batch_size, mini_batch_size):
        self.data_length = len(data_source)
        self.mini_batch_size = mini_batch_size
        self.batch_size = batch_size

    def __iter__(self):
        indices = th.arange(0, self.data_length).tolist()
        # Remove the remainder at the end of the dataset.
        num_batches = self.batch_size // self.mini_batch_size
        start_indices = th.randperm(self.data_length).tolist()
        batch_inds = []
        i = 0
        while len(batch_inds) < self.batch_size:
            start = start_indices[i]
            batch_inds.extend(indices[start:(start+self.mini_batch_size)%self.data_length])
            i+=1
        yield batch_inds[:self.batch_size]
...
def make_data_loader(
    ...
    if isinstance(transitions, types.TransitionsMinimal):
        ...
        data_loader_kwargs = data_loader_kwargs if data_loader_kwargs else {}
        if 'random_batch' in data_loader_kwargs:
            if data_loader_kwargs['random_batch']:
                return th_data.DataLoader(
                    transitions,
                    batch_sampler=RandomSequentialSampler(transitions,
                                                          batch_size=batch_size,
                                                          mini_batch_size=data_loader_kwargs['expert_mini_batch']),
                    collate_fn=types.transitions_collate_fn,
                )
        if "shuffle" not in data_loader_kwargs:
            data_loader_kwargs['shuffle'] = True
        if "drop_last" not in data_loader_kwargs:

...
class RewardNetSum(reward_nets.RewardEnsemble):
    def __init__(self, bases: List[reward_nets.RewardNet]):
        members = [RewardNetFromDiscriminatorLogit(b) for b in bases]
        for b in bases:
            assert bases[0].observation_space == b.observation_space
            assert bases[0].action_space == b.action_space
        super().__init__(observation_space = bases[0].observation_space, action_space = bases[0].action_space, members = members)


class RewardNetSumArchive:
    def __init__(self, base: List[reward_nets.RewardNet]):
        # super().__init__(base[0])
        # self.reward_logits = [RewardNetFromDiscriminatorLogit(b) for b in base]
        self.base_list = base
        self.observation_space = [b.observation_space for b in base]
        self.action_space = [b.action_space for b in base]
        self.normalize_images = [b.normalize_images for b in base]
    ...
    def forward(
        ...
        for base in self.base_list:
            logits = base.forward(state, action, next_state, done)
            summer = summer + -F.logsigmoid(-logits)
        return summer

class WassersteinReward(reward_nets.RewardNet):
    def __init__(self, base: reward_nets.RewardNet, reward_type: str = 'exp'):
        """Builds LogSigmoidRewardNet to wrap `reward_net`."""
        # TODO(adam): make an explicit RewardNetWrapper class?
        super().__init__(
            observation_space=base.observation_space,
...
class BCRewardGAIL(RewardNetFromDiscriminatorLogit):
    def __init__(self, base: reward_nets.RewardNet,
                 augmented_lambda: int,
                 bc_coefficient: float = 1e-3):
        self.augmented_lamda = augmented_lambda
        super().__init__(base)
        self.bc_reward = None
    ...
    def forward(
        ...
        logits = super().forward(state, action, next_state, done)
        if self.bc_reward:
            bc_rew = self.bc_reward * self.bc_coefficient
        else:
            raise NotImplementedError
        return logits*(1-self.augmented_lamda) + bc_rew*self.augmented_lamda

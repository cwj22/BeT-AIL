...
class OptionTrajectoryAccumulator(TrajectoryAccumulator):
    def finish_trajectory(
        self,
        key: Hashable,
        terminal: bool,
        ...
        traj = types.TrajectoryWithRewAndOpts(**out_dict_stacked, terminal=terminal)
        assert traj.rews.shape[0] == traj.acts.shape[0] == traj.obs.shape[0] - 1 == traj.opts.shape[0] -1
        return traj
    ...
    def add_steps_and_auto_finish(
            ...
            opts: np.ndarray,
            infos: List[dict],
    ) -> List[types.TrajectoryWithRewAndOpts]:
        """Calls `add_step` repeatedly using acts and the returns from `venv.step`.
        ...
        Returns:
            ...
            each `True` in the `dones` argument.
        """
        trajs: List[types.TrajectoryWithRewAndOpts] = []
        for env_idx in range(len(obs)):
            assert env_idx in self.partial_trajectories
            assert list(self.partial_trajectories[env_idx][0].keys()) == ["obs", "opts"], (
                "Need to first initialize partial trajectory using "
                "self._traj_accum.add_step({'obs': ob}, key=env_idx)"
            )


        zip_iter = enumerate(zip(acts, obs, opts, rews, dones, infos))
        for env_idx, (act, ob, op, rew, done, info) in zip_iter:
            if done:
                # When dones[i] from VecEnv.step() is True, obs[i] is the first
                # observation following reset() of the ith VecEnv, and
                # infos[i]["terminal_observation"] is the actual final observation.
            ...
            if done:
                ...
                # When done[i] from VecEnv.step() is True, obs[i] is the first
                # observation following reset() of the ith VecEnv.
                self.add_step(dict(obs=ob, opts =op), env_idx)
        return trajs
...
def generate_trajectories(
    ...
    *,
    ...
    if initial_option is not None:
        trajectories_accum = OptionTrajectoryAccumulator()
        opts = np.zeros((obs.shape[0],1), dtype=np.int64) + initial_option
    else:
        trajectories_accum = TrajectoryAccumulator()
        opts = np.zeros((obs.shape[0],1), dtype=np.int64)


    for env_idx, (ob,op) in enumerate(zip(obs, opts)):
        # Seed with first obs only. Inside loop, we'll only add second obs from
        # each (s,a,r,s') tuple, under the same "obs" key again. That way we still
        # get all observations, but they're not duplicated into "next obs" and
        # "previous obs" (this matters for, e.g., Atari, where observations are
        # really big).
        if isinstance(trajectories_accum, OptionTrajectoryAccumulator):
            trajectories_accum.add_step(dict(obs=ob, opts =op), env_idx)
        else:
    ...
    while np.any(active):
        acts, opts = get_actions(obs, opts)
...
def flatten_trajectories(
    ...
    for traj in trajectories:
        ...
        if gen_policy_observation_extension != 0:
            obs = traj.obs[:, :-gen_policy_observation_extension]
        else:
            obs = traj.obs
...
def flatten_trajectories_with_opts(
    trajectories: Sequence[types.Trajectory],
) -> types.Transitions:
    """Flatten a series of trajectory dictionaries into arrays.
    ...
    Returns:
        The trajectories flattened into a single batch of Transitions.
    """
    keys = ["obs", "next_obs", "acts", "dones", "infos", "opts", "next_opts", 'rews']
    parts: Mapping[str, List[np.ndarray]] = {key: [] for key in keys}
    for traj in trajectories:
        parts["acts"].append(traj.acts)
        ...
        opts = traj.opts
        parts["opts"].append(opts[:-1])
        parts["next_opts"].append(opts[1:])
    ...
    lengths = set(map(len, cat_parts.values()))
    assert len(lengths) == 1, f"expected one length, got {lengths}"
    return types.TransitionsWithRewAndOpts(**cat_parts)

def get_course_v_from_infos(trajectory, key = 'course_v'):
    if 'state' in trajectory.infos[0]: # this means this is gts env
        return np.array([info['state'][key] for info in trajectory.infos])
    else:
        mapping = {'course_v': 's', 'x': 'X', 'y': 'Y', 'z': None}
        if key == 'z':
            value = np.zeros_like([info[mapping['x']] for info in trajectory.infos])
        else:
            value = np.array([info[mapping[key]] for info in trajectory.infos])
        return value

def flatten_trajectories_with_rew(
    trajectories: Sequence[types.TrajectoryWithRew],gen_policy_observation_extension=0

) -> types.TransitionsWithRew:
    transitions = flatten_trajectories(trajectories,gen_policy_observation_extension)
    rews = np.concatenate([traj.rews for traj in trajectories])
    driver_num = np.concatenate([np.zeros_like(traj.rews) + i for i,traj in enumerate(trajectories)])

    return types.TransitionsWithRew(**dataclasses.asdict(transitions), rews=rews,)

def flatten_trajectories_with_rew_and_opt(
    trajectories: Sequence[types.TrajectoryWithRew],
    options: np.ndarray,
) -> types.TransitionsWithRew:
    ...
    rews = np.concatenate([traj.rews for traj in trajectories])
    driver_num = np.concatenate([np.zeros_like(traj.rews) + i for i,traj in enumerate(trajectories)])
    option_trans = np.concatenate([o[:-1] for o in options])
    next_option_trans = np.concatenate([o[1:] for o in options])

    kwargs = {}
    for k in ['course_v', 'x', 'y', 'z']:
        if hasattr(trajectories[0], k):
            kwargs[k] = np.concatenate([getattr(traj, k) for traj in trajectories])
        else:
            kwargs[k] = np.concatenate([get_course_v_from_infos(traj, k) for traj in trajectories])
    return types.TransitionsWithRewAndOpts(**dataclasses.asdict(transitions), rews=rews,
                                    opts = option_trans, next_opts=next_option_trans, driver_num=driver_num,
                                           **kwargs)
